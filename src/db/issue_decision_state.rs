...
pub struct IssueDecisionState {
    ...
    pub data: IssueDecisionStateData,
}
...
// will surely be extended with reversibility and more
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct IssueDecisionStateData {
    pub resolution: Resolution,
}
...
pub async fn insert_issue_decision_state(
    ...
    data: &IssueDecisionStateData,
) -> Result<()> {
    tracing::trace!("insert_issue_decision_state(issue_id={})", issue_number);
    let issue_id = *issue_number as i64;

    db.execute(
        "INSERT INTO issue_decision_state (issue_id, initiator, start_date, end_date, current, history, data) VALUES ($1, $2, $3, $4, $5, $6, $7) 
            ON CONFLICT DO NOTHING",
        &[&issue_id, &initiator, &start_date, &end_date, &serde_json::to_value(current).unwrap(), &serde_json::to_value(history).unwrap(), &serde_json::to_value(data).unwrap()],
    )
    .await
...
pub async fn update_issue_decision_state(
    ...
    db.execute("UPDATE issue_decision_state SET end_date = $2, current = $3, history = $4, data = $5 WHERE issue_id = $1", 
        &[&issue_id, &end_date, &serde_json::to_value(current).unwrap(), &serde_json::to_value(history).unwrap(), &serde_json::to_value(data).unwrap()]
    )
    .await
...
fn deserialize_issue_decision_state(row: &tokio_postgres::row::Row) -> Result<IssueDecisionState> {
    ...
    let data: IssueDecisionStateData = serde_json::from_value(row.try_get(6).unwrap())?;
